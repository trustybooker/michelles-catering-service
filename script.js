...
inquiryForm?.addEventListener('submit', async (event) => {
  ...
  button.disabled = true; button.textContent = 'Sending…'; const cart = JSON.parse(localStorage.getItem('michelles-quote-cart') || '[]'); if (cart.length) data.message = `Selected menus: ${cart.join(', ')}\n${data.message || ''}`.trim(); const smsBody = `Michelle's Catering inquiry\nName: ${data.name}\nEmail: ${data.email}\nDate: ${data.date}\nGuests: ${data.guests}\nEvent: ${data.eventType}\nDetails: ${data.message || 'None provided'}`;
